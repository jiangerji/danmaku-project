...
public class LibVLC {
	...
	private int aout = LibVlcUtil.isGingerbreadOrLater() ? AOUT_OPENSLES
	...
	public void setAout(int aout) {
		if (aout < 0)
			this.aout = LibVlcUtil.isICSOrLater() ? AOUT_OPENSLES
					: AOUT_AUDIOTRACK_JAVA;
		else
			this.aout = aout;
	}
	...
	public void setChroma(String chroma) {
		this.chroma = chroma.equals("YV12")
				&& !LibVlcUtil.isGingerbreadOrLater() ? "" : chroma;
	}
	...
	public native static void sendMouseEvent(int action, int button, int x,
			int y);
